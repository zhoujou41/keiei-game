...
(function () {
  ...
  var SPAWN_INTERVAL_MIN = 500; // 客の入店間隔（ランダム、席が増えたぶん短縮）
  var SPAWN_INTERVAL_MAX = 1100;
  ...
  var WORLD = {
    ...
    stovePoint: { x: 90, y: 90 },
    counterPoint: { x: 195, y: 110 },
    staffHomePoint: { x: 330, y: 110 },
  };
  ...
  var FURNITURE_LAYOUT = [
    ...
    { key: "fridge", x: 30, y: 60, w: 22, h: 60 },
    { key: "stove", x: 90, y: 60, w: 32, h: 60 },
    // キッチンを広くし、コックが料理を置く・ウェイターが受け取るための横長カウンターにした。
    { key: "counter", x: 195, y: 84, w: 74, h: 40 },
  ]
    .concat(
      TABLE_XS.reduce(function (acc, tx) {
        acc.push({ key: "table", x: tx, y: 225, w: 48, h: 90 });
        [tx - 34, tx + 34].forEach(function (cx) {
          [160, 193].forEach(function (cy) {
            acc.push({ key: "chair", x: cx, y: cy, w: 26, h: 50 });
          });
        });
        return acc;
      }, [])
    ...
    .concat([
      { key: "plant", x: 615, y: 345, w: 36, h: 36 },
      // 装飾（当たり判定なし。什器と違い経路計画には一切関与しない）。
      { key: "rug", x: 400, y: 330, w: 340, h: 60 },
      { key: "picture", x: 300, y: 27, w: 42, h: 26 },
      { key: "picture", x: 470, y: 27, w: 42, h: 26 },
    ]);
  ...
  function spawnCustomer() {
    ...
    var ent = {
      ...
      lookAroundPoint: {
        x: LOOK_BASE_POINT.x + (Math.random() * 70 - 35),
        y: LOOK_BASE_POINT.y + (Math.random() * 30 - 15),
      },
      dishEl: null,
      bubbleEl: null,
      g: elNS("g", {}),
  ...
  // 卓と卓の間、および卓列の両端の「安全に縦移動できるx」。TABLE_XS（260/400/540）と
  // 卓の半幅(24)から見て、確実に卓の外側になる値を選んでいる。
  var CORRIDOR_XS = [150, 350, 490, 610];
  ...
  // NOTICE_CUSTOMERに入った時の「！」の合図、CASHIERに入った時のコイン表示等、
  // 状態遷移そのものに追加したい小演出はここでstate変化を監視して付与する
  // （FSM側は純粋な状態管理のみなので、演出の都合はscene側に置く）。
  ...
  function announceStateChanges() {
    Object.keys(entities).forEach(function (id) {
      ...
      if (ent.state === WaiterState.NOTICE_CUSTOMER) showBubble(ent, "❕");
      else if (ent.state === CustomerState.ORDERING) showBubble(ent, "📝");
      else if (ent.state === WaiterState.CASHIER) showBubble(ent, "💴");
